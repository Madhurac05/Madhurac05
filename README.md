## Hi 👋, I'm Madhura Chilakandi

# about me 
💻 Final Year CSE Student | 🌐 Frontend and Backend developer | 🤖 AI/ML Enthusiast

# About Me
Final Year B.E. Computer Science Student
Angadi Institute of Technology and Management

💡 Driven by a passion for solving real-world challenges through Machine Learning
I enjoy transforming ideas into impactful solutions using data and intelligent systems.

🧠 Machine Learning Enthusiast
Hands-on experience with:

Frameworks: TensorFlow, PyTorch, scikit-learn
Tools: SpeechRecognition, pyttsx3

Building end-to-end ML pipelines for practical applications

🌍 Full-Stack Developer (MERN + Flask)
Comfortable working across both front-end and back-end using:

Flask, HTML/CSS, JavaScript
REST APIs and modern web app development practices

🚀 Constant Learner & Innovator
Always exploring cutting-edge technologies and developing creative projects to push the boundaries of what’s possible.


📍 Location: Belagavi, Karnataka, India

# My Projects 
 🤖 Black Line Following & Obstacle Avoidance Robot
Designed and developed an autonomous robot using microcontrollers, IR sensors, and ultrasonic sensors.
Features: Real-time line tracking & obstacle avoidance with decision-making algorithms.
Skills Gained: Embedded systems, robotics, sensor integration — valuable for AI, IoT & automation.

 🧠 IRIS: Intelligent Responsive Interface Solution (Desktop Assistant)
A voice-controlled desktop assistant to control apps, search the web, send emails, play music, show weather updates & more.
Tech: Python, Speech Recognition, Text-to-Speech
Highlights: Fast, privacy-focused, user-friendly interface.
Skills Gained: Python, AI, software development, user interaction design.


# Skill and Tools
💻 Languages:
C++, python , c 

📚 Frameworks & Libraries:
 • SpeechRecognition • pyttsx3 • Flask • OpenAI API  • Scikit-Learn

🧰 Tools & Platforms:
Git • GitHub • VS Code • Google Colab • Jupyter • Kaggle

🎨 Others:
Matplotlib • Seaborn • Flask • Streamlit • HTML5 • CSS3

# lt's Connect
🔗 LinkedIn : https://www.linkedin.com/in/madhura-chilakandi-5ab103319

🐙 GitHub : https://github.com/Madhurac05

📧 Email Me : madhurachilakandi2004@gmail.com

                                      

<!--
**Anandck07/Anandck07** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

<!--
**Madhurac05/Madhurac05** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
